...
int main(){
	...
	rc_mpu_set_dmp_callback(&mobilebot_controller); // callback function for imu attached to digital motion processor (dmp). Every time it runs, it publishes the lcm message
...
void read_mb_sensors(){
    ...
    // Read encoders    
    mb_state.left_encoder_delta = rc_encoder_read(LEFT_MOTOR) * LEFT_ENCODER_POLAR;
    mb_state.right_encoder_delta = rc_encoder_read(RIGHT_MOTOR) * RIGHT_ENCODER_POLAR;
...
void mobilebot_controller(){
    ...
    publish_mb_msgs(); // publish message everytime it gets callbacked
    // printf("%f", mb_state.left_cmd);
